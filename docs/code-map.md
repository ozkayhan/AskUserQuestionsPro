...
| `server/server.js`                       | `node:http` server (port `ASKUSER_PORT` or 4517). Routes `/health` `/current` `/events` `/ask` `/resume` `/answer` `/cancel` + static files from `../web`. Exports `server`, `bridge`.                  | Change HTTP endpoints, SSE, static serving, request limits.                                   |
| `server/bridge.js`                       | `Bridge` class: single-flight `_pending` + `_seq`, bounded detach state, and completed-answer cache. Methods `submitQuestions` `peek` `getCurrent` `provideAnswers` `cancel` `detach` `waitForAnswers`. | Change question/answer coordination semantics.                                                |
