...
| `POST /draft`    | `{ id, capability, revision, answers: {...} }` | `{ ok, revision, replayed }` or typed error                    | Persist each material browser edit immediately when the capability and expected revision match. Writes are serialized so revisions remain ordered; identical retries are idempotent and a different stale edit returns `stale_revision`.                                                                                                                                          |
...
```jsonc
{
  ...
  "answers": {/* opaque plain object; server stores and returns as-is */},
}
```
...
```jsonc
{
  "answers": {/* the plain object the browser POSTed to /answer */},
}
```
