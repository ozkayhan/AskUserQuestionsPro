...
| `POST /draft`    | `{ id, capability, revision, answers: {...} }` | `{ ok, revision, replayed }` or typed error                    | Persist a browser draft only when the capability and expected revision match. Identical retries are idempotent; a different stale edit returns `stale_revision`.                                                                                                                                          |
