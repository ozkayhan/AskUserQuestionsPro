...
| `POST /rounds/:roundId/cancel` | `{ reason? }`                                  | `{ ok: true, roundId, reason }` (200) or typed error                  | Explicit host control for cancelling one active `drafting`, `detached`, or `reconnecting` round. The exact durable id is required; delivery-pending/uncertain results are not silently discarded.                                                                                                                                                                                                                                                                                                                  |
...
## MCP tools: `mcp__askuserquestionspro__ask`, `mcp__askuserquestionspro__resume`, `mcp__askuserquestionspro__list_recoverable_rounds`, and `mcp__askuserquestionspro__cancel_round`
