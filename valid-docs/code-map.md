...
|------|-----------|-------------|
| `bin/cli.js` | CLI entry (`askuserquestionspro`). Subcommands: `init`, `install`, `uninstall`, `serve`, `mcp`, `doctor`, `help`. | Change CLI commands, install/uninstall flow, `doctor` health checks. |
| `bin/install.js` | Pure functions `addHook()` / `removeHook()` that mutate a `settings.json` object. Constants `MATCHER='AskUserQuestion'`, `TIMEOUT=360`. | Change how the hook entry is added/removed or conflict detection. |
...
| `mcp-server/askuserquestionspro-mcp.mjs` | JSON-RPC 2.0 stdio MCP server. Defines `ASK_TOOL`, handles `initialize`/`tools/list`/`tools/call`/`ping`. `handleAsk()` routes to bridge-client (30 min timeout). | Change the `ask` tool schema or MCP protocol handling. |
...
| `hooks/askuserquestionspro-bridge.mjs` | The `PreToolUse` hook script. Reads stdin, honors `ASKUI_FORCE_MCP`, calls bridge-client (5 min timeout), falls back to native on error. | Change hook behavior / the force-MCP opt-in. |
...
| `web/app.js` | `Flow` component — state machine: answers state, navigation, keyboard shortcuts, submit. | Change navigation/keyboard/submit behavior. |
| `web/live.js` | `useLiveQuestions()` (SSE from `/events`) and `postAnswers()` (POST `/answer`). | Change client↔server transport. |
| `web/views.js` | Presentational components: `Sidebar`, `QItem`, `SidebarGrouped`, `SidebarSearch`, `Hints`, `QuestionCard`, `CustomPopup`, `Summary`, `ThemeSwitcher`. | Change UI layout/markup. |
...
| `web/themes.js` | Theme registry (`amoled`, `paper`, `phosphor`, `dusk`, `aurora`), `read/apply/current/swapFont`, `KNOWN_TOKENS`. | Add/edit themes or token handling. |
