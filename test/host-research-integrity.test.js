...
test('candidate research has a record, dated source state, and next gate', () => {
  ...
  const records = fs.readFileSync(path.join(dir, 'candidates.md'), 'utf8');
  assert.match(readme, /temporary `HOME`/);
  assert.match(readme, /never\ninstalls candidate hosts/);
  for (const h of ledger.hosts) {
    assert.match(records, new RegExp('`' + h.id + '`'));
    assert.ok(h.status === 'Researching' || h.status === 'Unsupported');
    assert.ok(h.nextGate && h.nextGate.length > 10);
    if (h.evidenceClass.includes('official-doc'))
      assert.ok(h.sources.length, `${h.id} source missing`);
    assert.doesNotMatch(
      JSON.stringify(h),
      /synthetic-(question|answer)|password|token\s*[:=]|\/Users\/oka/i
    );
  }
});
