...
test('native evidence has one metadata-complete result per OS and scenario', () => {
  ...
  for (const row of evidence.oses) {
    for (const key of ['os', 'architecture', 'node', 'configRoot', 'command', 'date', 'result', 'limitation', 'scenarioResults']) assert.ok(row[key], `${row.id}/${key}`);
    for (const scenario of evidence.scenarios) assert.ok(row.scenarioResults[scenario], `${row.id}/${scenario}`);
  }
...
test('supported host promotion requires native OS and lifecycle evidence', () => {
  for (const phrase of ['installed conformance', 'long-round', 'install/upgrade', 'trust', 'config-scope']) assert.match(doc, new RegExp(phrase));
