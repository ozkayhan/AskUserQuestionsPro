...
function cli(...args) {
  ...
  if (result.error?.code === 'ENOENT') throw new Error('playwright-cli is required; install it and ensure it is on PATH');
...
async function main() {
  const server = spawn(process.execPath, ['server/server.js'], { cwd: path.join(__dirname, '..'), env: { ...process.env, ASKUSER_PORT: String(port), XDG_CONFIG_HOME: config }, stdio: ['ignore', 'ignore', 'pipe'] });
  let stderr = '';
  ...
  try {
    ...
    cli('resize', '320', '480');
    assert.equal(evaluate("(() => { const d=document.querySelector('[role=dialog]'); return d.scrollHeight > d.clientHeight; })()"), true);
