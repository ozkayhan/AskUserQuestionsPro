...
const { createDraftWriter, readPendingDraft, readLatestPendingDraft, clearPendingDrafts, reconcileDraft } = require('../web/draft-writer.js');
...
test('draft writer clears all obsolete local revisions after server selection', () => {
  const entries = new Map([
    ['askuserquestionspro:draft:r:1', 'old'],
    ['askuserquestionspro:draft:r:4', 'new'],
    ['unrelated', 'keep'],
  ]);
  const storage = {
    get length() { return entries.size; },
