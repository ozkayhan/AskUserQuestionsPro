...
const { adapterEnabled, diagnosticsPolicy, deliveryPolicy, closurePolicy } = require('../lib/runtime-settings.cjs');
...
test('runtime matrix owner hooks change behavior for every runtime field', () => {
  ...
  assert.deepStrictEqual(diagnostics.redact({ boundary: 'hook', path: '/secret/file' }), { boundary: 'hook' });
