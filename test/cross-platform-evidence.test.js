...
test('native OS evidence has scenario parity and honest gaps', () => {
  ...
  for (const os of ['macOS arm64', 'Linux native', 'Windows native']) assert.match(doc, new RegExp(`\\| ${os} \\|`));
  assert.match(doc, /WSL.*does not count/i);
});
test('cross-platform evidence requires metadata and redaction', () => {
  for (const field of ['architecture', 'Node version', 'config root', 'exact command', 'date', 'result', 'limitation']) assert.match(doc, new RegExp(field, 'i'));
...
test('cross-platform structured rows cover every OS/scenario', () => {
  for (const row of evidence.oses) assert.equal(Object.keys(row.scenarioResults).length, evidence.scenarios.length);
});
