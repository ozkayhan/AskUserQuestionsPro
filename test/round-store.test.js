...
test('store writes private snapshots and reloads the newest record', () => {
  ...
  const created = store.create({ questions: [{ question: 'Q?' }], capability: 'cap', retentionMs: 1000 });
...
test('cleanup removes only expired records', () => {
  const { store } = fixture();
  const expired = store.create({ questions: [{ question: 'old' }], capability: 'a', retentionMs: 1 });
  const fresh = store.create({ questions: [{ question: 'new' }], capability: 'b', retentionMs: 1000 });
  const later = new RoundStore({ root: store.root, now: () => 102 });
  assert.deepEqual(later.cleanupExpired(), [expired.record.roundId]);
