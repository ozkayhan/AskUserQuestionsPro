...
function App() {
  ...
  const [answers, setAnswers] = useState(() => {
    const a = {};
    QUESTIONS.forEach((q) => {
      a[q.id] = { sel: [], confirmed: false, customText: '' };
    });
    return a;
  });
  ...
  const activate = useCallback(
    (qIndex, optIdx) => {
      ...
      if (q.multiSelect) {
        ...
        if (inSel) {
          if (isCustom) {
            setPopup({ qid: q.id, optIdx, draft: a.customText });
            return;
          }
      ...
      <main className="inspector">
        ...
        {!isSummary && (
          <footer className="hints">
            <span className="hint">
              <span className="kbd-group">
                <Kbd>1</Kbd>–<Kbd>{fullOptions(QUESTIONS[current]).length}</Kbd>
              </span>
              Select
            </span>
            <span className="hint">
  ...
  return (
    <div className="qcard" data-motion={motion} data-dir={dir}>
      ...
      <p className="qcard__meta">
        {q.multiSelect ? 'Select all that apply.' : 'Select one option.'} An “Other” choice is
        always available.
      </p>
      ...
      <div className="popup">
        ...
        <div className="popup__foot">
          ...
          <div className="popup__actions">
            ...
            <button className="btn btn--primary" onClick={onSave} disabled={!draft.trim()}>
              Save answer
            </button>
          </div>
        </div>
                ...
                <div className="srow__a">
                  {a.sel.length === 0 ? (
                    ...
                    a.sel.map((s) => {
                      ...
                      return (
                        <span key={s} className={'tag' + (o.custom ? ' tag--custom' : '')}>
                          {val}
                        </span>
                      );
                    })
  ...
  return (
    <TweaksPanel>
      ...
      <TweakColor
        label="Accent"
        value={t.accent}
        options={['#0070f3', '#ededed', '#1ed760', '#f5a623', '#bd34fe']}
