...
| **13** | `install.sh` baştan düzeltildi: dosyalar kalıcı `~/.local/share/askuserquestionspro`'ye kopyalanıyor (B1, temp dir silinme bug'ı giderildi); jq `any(...)` ile idempotent (B2, mükerrer hook satırı önlendi); hook yolu çift tırnaklı (B7). `bash -n` geçti.                   | `28c8860`             |
