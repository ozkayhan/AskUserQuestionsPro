...
| source-edit-policy | Shared validator computes baseline-relative tracked worktree/index paths and untracked paths, then rejects every new unauthorized path. Only declared maintained docs, planning metadata, Phase 18 artifacts, and the validator baseline are allowed; Phase 17 paths already present in the capture remain permitted. | Unauthorized tracked worktree/index/untracked changes fail |
