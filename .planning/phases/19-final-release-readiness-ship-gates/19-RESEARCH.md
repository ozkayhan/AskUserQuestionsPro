...
1. **What exact commit/version is the v1.1.1 release candidate?** Current manifest is 1.1.0; resolve before final ship decision. [VERIFIED: package.json and milestone docs]
2. **Are external unavailable lanes a blocker to publication or only a blocker to compatibility promotion?** Requirements demand honest handoff, but the final Ready/Blocked policy should state the product-owner decision explicitly. [VERIFIED: REQUIREMENTS.md; interpretation [ASSUMED]]
3. **Which supported macOS installer targets must be run manually beyond the deterministic fake-host tests?** The repository documents native macOS/Linux/Windows expectations but local tests cannot establish all native behavior. [VERIFIED: docs/maintenance.md and handoff]
