...
| UAT-01 | `16-UAT-MATRIX.md` source-link, status, redaction, and handoff-field audit | One matrix; all links resolve; PASS/PARTIAL/UNAVAILABLE are bounded; each PARTIAL/UNAVAILABLE row has owner, environment, action/next gate | No unsupported host/native/browser claims are accepted |
...
| UAT-01/UAT-02 | `git diff --check`, production-dependency drift, protected-file checks | Exit 0; no protected config/gitignore changes or staging | Fail on unexpected changes |
