...
function migrateLegacy(bytes, parsed) {
  ...
  try {
    ...
    return { ...inspected, migration: { needed: true, backup: false, failed: true, code: error.code || 'MIGRATION_FAILED' } };
  }
}

function inspect() {
  const bytes = readRaw();
  if (!bytes) return { status: 'missing', revision: null, hash: null, effective: Schema.envelopeDefaults(), migration: null };
  const hash = revision(bytes);
  let parsed;
  try { parsed = JSON.parse(bytes.toString('utf8')); } catch { return { status: 'invalid', revision: hash, hash, effective: Schema.envelopeDefaults(), migration: null }; }
  ...
  return { ...result, revision: currentHash, hash: currentHash, effective: result.envelope || Schema.envelopeDefaults(), migration: result.migration || { needed: false, backup: false } };
}
...
function load() { return inspect(); }

function writeEnvelope(envelope, expectedRevision) {
  const current = inspect();
  if (expectedRevision !== undefined && expectedRevision !== current.revision) return { ok: false, code: 'STALE_REVISION', status: current };
  if (!envelope || envelope._v !== V2) return { ok: false, code: 'INVALID_SETTINGS', status: current };
  const next = Schema.validateEnvelope(envelope);
  try {
    fs.mkdirSync(DIR, { recursive: true, mode: 0o700 });
    writeFileAtomic(FILE, JSON.stringify(next, null, 2) + '\n');
    return { ok: true, value: next, status: inspect() };
  } catch (error) { log('settings', error); return { ok: false, code: error.code || 'WRITE_FAILED', error, status: current }; }
}

function mutateCompareAndSwap(expectedRevision, mutator) {
  const current = inspect();
  if (current.status === 'invalid' || current.status === 'unsupported-future') return { ok: false, code: 'UNSAFE_SOURCE', status: current };
  if (expectedRevision !== undefined && expectedRevision !== current.revision) return { ok: false, code: 'STALE_REVISION', status: current };
...
function doctorProjection(status = inspect()) {
  ...
  const projection = {
    ...
    migration: status.migration ? { needed: !!status.migration.needed, backup: !!status.migration.backup } : null,
    effective: {
      browser: { strategy: e.browser.strategy, theme: e.browser.theme, uiScale: e.browser.uiScale, highContrast: e.browser.highContrast, reduceMotion: e.browser.reduceMotion },
...
function inspectReadOnly() {
  ...
  return { ...result, revision: hash, hash, effective: result.envelope || Schema.envelopeDefaults(), migration: result.migrated ? { needed: true, backup: false } : { needed: false, backup: false } };
}

module.exports = { read, write, load, inspect, inspectReadOnly, writeEnvelope, mutateCompareAndSwap, getPath, doctorProjection, BACKUP };
