...
function writeEnvelope(envelope, expectedRevision) {
  ...
  if (!envelope || envelope._v !== V) return { ok: false, code: 'INVALID_SETTINGS', status: current };
...
function mutateCompareAndSwap(expectedRevision, mutator) {
  ...
  if (expectedRevision !== undefined && expectedRevision !== current.revision) return { ok: false, code: 'STALE_REVISION', status: current };
  const next = mutator(Schema.envelopeDefaults ? current.effective : current.effective);
...
module.exports = { read, write, load, inspect, writeEnvelope, mutateCompareAndSwap, getPath };
