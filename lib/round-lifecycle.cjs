...
function createLifecycle({
  ...
  let settings = null;
  try {
    const inspected = require('./settings.js').inspect();
    settings = inspected.status === 'current' ? inspected.effective.diagnostics : null;
  } catch { settings = null; }
  const startedAt = finite(now(), Date.now());
  const adapterName = String(adapter);
  // Every diagnostic identifies an allowlisted boundary even when a caller
  // only supplies its adapter. Unknown adapters stay attributed to the bridge
  // rather than emitting an unvalidated value.
  const defaultBoundary = BOUNDARIES.has(adapterName) ? adapterName : 'bridge';
  let finished = false;
  ...
  function emit(event, reason, details = {}) {
    const payload = {
      ...
      boundary: BOUNDARIES.has(details?.boundary) ? details.boundary : defaultBoundary,
      deadlineOwner: DEADLINE_OWNERS.has(details?.deadlineOwner)
        ? details.deadlineOwner
        : 'none',
    };
