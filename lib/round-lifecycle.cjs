...
const { diagnosticsPolicy } = require('./runtime-settings.cjs');
...
function createLifecycle({
  ...
  function emit(event, reason, details = {}) {
    ...
    details = diagnostics.redact(details);
    const payload = {
      ...
      deadlineOwner: DEADLINE_OWNERS.has(details?.deadlineOwner) ? details.deadlineOwner : 'none',
    };
