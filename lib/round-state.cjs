...
function transition(record, event, { now, deadlineOwner, reason } = {}) {
  if (!record || !Number.isFinite(now) || now < record.updatedAt) {
    return { ok: false, code: 'invalid_timestamp', record };
  }
  if (deadlineOwner !== undefined && !DEADLINE_OWNERS.includes(deadlineOwner)) {
    return { ok: false, code: 'invalid_deadline_owner', record };
  }
  if (reason !== undefined && !TERMINAL_REASONS.includes(reason)) {
    return { ok: false, code: 'invalid_terminal_reason', record };
  }
  const state = TRANSITIONS[record.state]?.[event];
  if (!state) return { ok: false, code: 'illegal_transition', record };
