...
function pathsFor(home = resolveUserHome()) {
...
function installMcp({ home = resolveUserHome(), mcpPath, nodePath = process.execPath } = {}) {
...
function removeMcp({ home = resolveUserHome() } = {}) {
  const paths = pathsFor(home);
...
function deploySkill({ home = resolveUserHome(), source } = {}) {
...
function hasPlugin(home = resolveUserHome()) {
  const paths = pathsFor(home);
  return fs.existsSync(paths.pluginManifest) && fs.existsSync(paths.pluginSkill);
}

function removePlugin(home = resolveUserHome()) {
  const paths = pathsFor(home);
