...
export function parseManagedPids(lsofOutput, port) {
  const normalizedPort = String(port);
  if (!/^\d{1,5}$/.test(normalizedPort) || Number(normalizedPort) > 65535) return [];
  const portPattern = new RegExp(`(?:^|:)${normalizedPort}(?:$|->)`);
  const pids = new Set();
  let pid = null;
  for (const line of String(lsofOutput).split('\n')) {
    if (line.startsWith('p')) {
      const candidate = line.slice(1);
      pid = /^\d+$/.test(candidate) ? candidate : null;
      continue;
    }
    if (pid && line.startsWith('n') && portPattern.test(line.slice(1))) pids.add(pid);
  }
  return [...pids];
}
...
function managedPids(serverPath) {
  ...
  try {
    const output = execFileSync('lsof', ['-nP', '-a', '-iTCP', '-sTCP:LISTEN', '-Fpn'], {
      encoding: 'utf8',
    });
    return parseManagedPids(output, PORT).filter((pid) => {
      try {
