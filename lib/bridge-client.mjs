...
const { deliveryPolicy, closurePolicy } = require('./runtime-settings.cjs');
...
export function openBrowser() {
  ...
  if (browser.strategy === 'manual' || (browser.strategy === 'auto' && process.env.ASKUSER_OPEN_BROWSER === '0')) {
...
export async function askBridge(
  ...
  const delivery = deliveryPolicy(Settings.inspect());
  const closure = closurePolicy(Settings.inspect());
  ...
  try {
    ...
    lifecycle?.finish(closure.mode === 'after-delivery' ? 'completed' : 'completed', { deadlineOwner: delivery.mode === 'confirm' ? 'host' : 'none' });
    return body.answers;
  } catch (e) {
