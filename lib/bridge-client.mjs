 ...
 */
export async function askBridge(questions, { timeoutMs, signal, requestId, lifecycle, timerApi = globalThis } = {}) {
  ...
  try {
    ...
    if (e?.name === 'AbortError') {
      ...
      lifecycle?.finish('application_timeout', { boundary: 'bridge', deadlineOwner: 'application' });
      throw new TimeoutError(`askBridge timed out after ${timeoutMs}ms`, { deadlineOwner: deadlineExpired ? 'application' : 'transport' });
    }
...
/**
 * Retrieve the latest detached round (or a specific requestId) after a host
 * process lost its original MCP connection. The browser round stays owned by
 * the bridge until the bounded detached-round TTL expires.
 */
export async function resumeBridge(selector, { timeoutMs = 60 * 60 * 1000, signal, timerApi = globalThis } = {}) {
  const controller = new AbortController();
  ...
  try {
    const r = await fetch(`${BASE}/resume`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(
        typeof selector === 'string' ? { requestId: selector } : selector && typeof selector === 'object' ? selector : {}
      ),
      signal: controller.signal,
    });
    const body = await r.json().catch(() => ({}));
