...
const { resolveUserHome } = require('./host-platforms.cjs');
...
class RoundStore {
  constructor({ root, now = Date.now, fsImpl = fs } = {}) {
    this.root =
      root ||
      path.join(
        process.env.XDG_CONFIG_HOME || path.join(resolveUserHome(), '.config'),
        'askuserquestionspro'
      );
  ...
  _load() {
    for (const name of this.fs.readdirSync(this.roundsDir)) {
      ...
      try {
        ...
        if (['drafting', 'reconnecting'].includes(record.lifecycle.state)) {
          const retentionMs = Math.max(1, record.expiresAt - record.createdAt);
          const now = this.now();
          const detached = Record.transition(record, 'detach', record.revision, now, {
            deadlineOwner: 'host',
            expiresAt: now + retentionMs,
          });
          if (detached.ok && !detached.replayed) {
            try {
              writeFileAtomic(this._file(record.roundId), JSON.stringify(detached.record), {
                fsImpl: this.fs,
                mode: 0o600,
              });
              record = detached.record;
            } catch {
              // Keep the original snapshot in memory; a later retry can recover it.
            }
          }
        }
  ...
  remove(roundId) {
    ...
    if (!this._records.has(roundId)) return { ok: false, code: 'not_found' };
    try {
      this.fs.unlinkSync(this._file(roundId));
    } catch (error) {
      if (error.code === 'ENOENT') return { ok: false, code: 'not_found' };
      return { ok: false, code: 'persistence_error', error };
    }
    this._records.delete(roundId);
    return { ok: true };
  }
  ...
  list() {
    return [...this._records.values()]
      .filter((r) => r.expiresAt > this.now() || r.lifecycle.state === 'reconnecting')
      .map(Record.metadata);
  }
  recoverable() {
    return [...this._records.values()].filter(
      (record) =>
        (record.expiresAt > this.now() || record.lifecycle.state === 'reconnecting') &&
        !record.answers &&
  ...
  cleanupExpired({ protectedRoundIds = [] } = {}) {
    const protectedIds = new Set(protectedRoundIds);
    const removed = [];
    for (const [id, record] of this._records)
      if (
        record.expiresAt <= this.now() &&
        !protectedIds.has(id) &&
        !['drafting', 'reconnecting'].includes(record.lifecycle.state)
      ) {
        try {
